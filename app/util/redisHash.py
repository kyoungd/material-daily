...
import json
import os
from .utilRedis import RedisAccess
...
class JwtSysOp(RedisHash):
    def __init__(self):
        key = os.getenv('KEY_NAME_JWT_SYSOP', 'JWT_SYSOP')
        super().__init__(key)
    ...
    def run():
        try:
            candidate = JwtSysOp()
            jwt = candidate.getAll()
            value = jwt['JWT'.encode('ascii')]
            data = json.loads(value.decode('ascii'))
            return data['jwt']
